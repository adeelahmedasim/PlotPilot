...
const createProperty = async (req, res) => {
    try {
        ...
        // Validate input data
        if (!title || !description || !propertyType || !location || !price || !photo || !email) {
        ...
        try {
            ...
            throw error; // Rethrow the error after handling the transaction
        } finally {
            session.endSession(); // Ensure session is closed, regardless of success or failure
        }
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
